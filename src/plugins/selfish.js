/*
   Generalized selfish miner, capable of producing the full range of selfish strategies.
   Tune their behavior via the config {} object in strategy_manifest.
*/
...
export function invokeStrategyS(activeEvent, p, blocks) {
/*
   Entry point from the sim_core, and high level coordinator of the strategy.  Selfish pools have to 
   track three critical viewpoints: 1) Their belief of the state of the honest network; 2) Their own
   state, 3)their belief of what the honest network thinks about their own state.
*/
   log(`invokeStrategyS:   ${activeEvent.simClock.toFixed(7)} ${p.id} action: ${activeEvent.action}`);
